...
public class xlPackagingServiceLogic extends xlServiceLogic
{
	...
	//Added by RahulV on 17-June-2022 TO GET INSERT SQL STATEMENT TO MIGRATE SQL TABLE ROWS FROM SOURCE TO TARGET
	//private static String CALL_BW_GET_INSERT_DATA_SQL_OF_ALL_TABLES_TO_MIGRATE = "{CALL BW_GET_INSERT_DATA_SQL_OF_ALL_TABLES_TO_MIGRATE()}";
	private static String CALL_BW_GENERATE_SQL_INSERT_SCRIPT = "{CALL BW_GENERATE_SQL_INSERT_SCRIPT(?,?)}";
    private static String CALL_GET_SQL_TABLELIST_TO_MIGRATE_DATA = "SELECT TABLE_NAME, TABLE_DESC, ALL_COLUMN_NAMES, KEY_COLUMNS_NAMES FROM BW_TABLE_TO_MIGRATE";
				...
				switch(action)
				{
					...
					case 77:				//  GET_TEMPLATE_USING_BLOB_ID		 Get Template using Blob Id
						...
						try
						{
							...
							//System.out.println("after DownloadTemplateBlobForPackaging :  fetched blob : filename = " + args[0]);
							...
							responseBuffer = "SUCCESS" + ContentDelimeter +  "templatefile";
						}
						catch (Exception e)
						{
					...
					case 78:				//  GET SQL SQL TABLE LIST OF MIGRATION
						try
						{
							String sqlTableList = null;
							sqlTableList = GetListOfSQLTablesSetForMigration(connection);
							responseBuffer = "SUCCESS" + ContentDelimeter +  sqlTableList;
						}
						catch (SQLException e)
						{
							responseBuffer = "FAILURE" + ContentDelimeter +  e.getMessage() +  "\nFailed to get SQL Table List. Try again" + ContentDelimeter   ;
						}
						break;


					case 79:				//  GET SQL TABLE INSERT STATEMNT OF SINGLE TABLE
						//System.out.println("79.........GET_INSERT_SQL_STMT_FOR_SQL_TABLE");
						//System.out.println("buf : " +  buf); 
						String sqlTableName, whereClause;
						sqlTableName = st.nextToken(Seperator);
						whereClause = st.nextToken(Seperator);
						...
						try
						{
							String sqlInsertStmt = null;
							sqlInsertStmt = GetInsertTableStatementOfSQLTable(connection, sqlTableName, whereClause);
							responseBuffer = "SUCCESS" + ContentDelimeter +  sqlInsertStmt;
						}
						catch (SQLException e)
						{
							responseBuffer = "FAILURE" + ContentDelimeter +  e.getMessage() +  "\nFailed to get Insert SQL Statements for Table. Try again" + ContentDelimeter   ;
						}
						break;
...
/*
			...
			{
				responseBuffer = "FAILURE" + Seperator + e.getMessage() + Seperator +  "\n Try again" + ContentDelimeter;
			}
...
//		}
	...
	//Get SQL STATMENTS FOR SINGLE TABLES PASSED AS PARAMETER
	public String GetInsertTableStatementOfSQLTable(Connection connection, String tableName, String whereClause) throws SQLException
	{
		...
		try 
		{
			stmt = connection.prepareCall(CALL_BW_GENERATE_SQL_INSERT_SCRIPT);
			...
			while (rs.next()) 
			{				// InsertSQL, TableName
				//System.out.println(rs.getString(1) + Seperator + rs.getString(2) + ContentDelimeter);
				sb.append(rs.getString(1) + Seperator + rs.getString(2) + ContentDelimeter);
			}
			rs.close();
		...
		//System.out.println("Insert Row Buffer : " + sb.toString());
		return sb.toString();		
	}


	//Get SQL STATMENTS FOR ALL TABLES DEFINED IN BW_TABLE_TO_MIGRATE
	public String GetListOfSQLTablesSetForMigration(Connection connection) throws SQLException
	{
        ...
        try
        {
            preparedstatement = connection.prepareStatement(CALL_GET_SQL_TABLELIST_TO_MIGRATE_DATA);
            rs = preparedstatement.executeQuery();
            while ( rs.next() )
            {		//TABLE_NAME, TABLE_DESC, ALL_COLUMN_NAMES, PK_COLUMNS_NAMES
				//System.out.println(rs.getString(1) + ", " + rs.getString(2)  + ", " + rs.getString(3)  + ", " + rs.getString(4));
				sb.append(rs.getString(1) + Seperator + rs.getString(2) + Seperator  + rs.getString(3) + Seperator  + rs.getString(4) + ContentDelimeter);
            }

			if (sb.length() > 0)
			{
				sb.deleteCharAt(sb.length()-1);
//				sb.append(DataBlockSeperator);
			}
			rs.close();
        }
        catch( Exception e )
	...
	/*
			System.out.println("before calling  "  + CALL_BW_GET_INSERT_DATA_SQL_OF_ALL_TABLES_TO_MIGRATE);
			
			cstmt = connection.prepareCall(CALL_BW_GET_INSERT_DATA_SQL_OF_ALL_TABLES_TO_MIGRATE);
			cstmt.execute();

			System.out.println("after calling  "  + CALL_BW_GET_INSERT_DATA_SQL_OF_ALL_TABLES_TO_MIGRATE);
			System.out.println("");
			//Reading BW_TABLE_TO_MIGRATE  -> TABLE_NAME,DESC,ALL_COLUMNS,PK_COLUMNS
			rs1 = cstmt.getResultSet();
			while (rs1.next()) 
			{
				System.out.println(rs1.getString(1) + Seperator + rs1.getString(2) + Seperator  + rs1.getString(3) + Seperator  + rs1.getString(4) + ContentDelimeter);
				sb.append(rs1.getString(1) + Seperator + rs1.getString(2) + Seperator  + rs1.getString(3) + Seperator  + rs1.getString(4) + ContentDelimeter);
				System.out.println("");
			}
			sb.deleteCharAt(sb.length()-1);
			sb.append(DataBlockSeperator);
			...
			int loopCount = 1;
			System.out.println("");
			System.out.println("loopCount at start : " + loopCount);	
			System.out.println("After Rs1 : Insert Row Buffer : " + sb.toString());
			System.out.println("");
            ...
            while (cstmt.getMoreResults()) 
			{
				...
				//Reading SQL TABALE INSERT  -> INSERT_SQL,TABLE_NAME
				while(rs.next())
				{
					System.out.println(rs.getString(1) + Seperator + rs.getString(2) + ContentDelimeter);
					sb.append(rs.getString(1) + Seperator + rs.getString(2) + ContentDelimeter);
					System.out.println("sb inside loop : " + sb.toString());
					System.out.println("");
				}
				rs.close();
				System.out.println("AFter Rs at end of loop iteration .... Insert Row Buffer : " + sb.toString());
				System.out.println("");
			}
			sb.deleteCharAt(sb.length()-1);
			sb.append(DataBlockSeperator);
		...
		try 
		{
			...
			boolean results = stmt.execute(CALL_GET_SQL_TABLELIST_TO_MIGRATE_DATA );
			int rsCount = 0;
			...
			do 
			{
				if (results) 
				{
					...
					{
						System.out.println(rs.getString(1) + ", " + rs.getString(2)  + ", " + rs.getString(3)  + ", " + rs.getString(4));
						sb.append(rs.getString(1) + Seperator + rs.getString(2) + Seperator  + rs1.getString(3) + Seperator  + rs1.getString(4) + ContentDelimeter);
					}
					sb.deleteCharAt(sb.length()-1);
					sb.append(DataBlockSeperator);
					rs.close();
		...
		{
			System.out.println("Error occured in GetListOfSQLTablesSetForMigration." + e.getMessage());
		} 
		finally
		{
			try {
