...
public class TableManager{
	...
	private static String CALL_BW_GET_ID_FROM_CUBOID_PATH = "{CALL BW_GET_ID_FROM_CUBOID_PATH(?,?,?,?)}"; //Added by Lakshman on 20190530 for ID Independent Template
...
*/
    ...
    public static TableInfo getTableInfo( Connection connection, int a_user_id, int a_table_id ) throws SystemException {
		...
		System.out.println("Inside TableManager.getTableInfo()..");
		System.out.println("Inside TableManager.getTableInfo().. a_user_id : " + a_user_id);
		System.out.println("Inside TableManager.getTableInfo().. a_table_id : " + a_table_id);
...
//		}
	...
	public static int getTableIdFromPath(Connection connection, String cuboidPath) 
	{
		PreparedStatement ps = null;
		String neighborhoodHierarchy		= "";
		...
		System.out.println("Inside TableManager:getTableIdFromPath");

		try 
		{
			String[] cuboidPathItems = cuboidPath.split("/");
			//System.out.println("TableManager:getTableIdFromPath:cuboidPath Size: " + cuboidPathItems.length);

			if (cuboidPathItems.length == 4)
			{
				neighborhoodHierarchy	= cuboidPathItems[0];
				collaborationName		= cuboidPathItems[1];
				whiteboardName			= cuboidPathItems[2];
				cuboidName				= cuboidPathItems[3];
				System.out.println("neighborhoodHierarchy = " + neighborhoodHierarchy + ",collaborationName = " + collaborationName + ",whiteboardName = " + whiteboardName + ",cuboidName = " + cuboidName);

				ps = connection.prepareStatement(CALL_BW_GET_ID_FROM_CUBOID_PATH);
				ps.setString(1, neighborhoodHierarchy);
				ps.setString(2, collaborationName);
				ps.setString(3, whiteboardName);
				ps.setString(4, cuboidName);
				resultset = ps.executeQuery();

				if (resultset.next())
				{
					tableId = resultset.getInt("TABLE_ID");
					System.out.println("TableManager:getTableIdFromPath:tableId " + tableId);
				}
			}
		}
		catch( Exception e ) 
