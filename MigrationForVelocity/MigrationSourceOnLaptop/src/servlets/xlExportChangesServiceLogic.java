...
public class xlExportChangesServiceLogic extends xlServiceLogic
{
    ...
    public xlExportChangesServiceLogic(xlExportChangesService srv) {
        super(srv);
    }
		...
		for (int ti = 0; ti < MAX_RETRY_ATTEMPTS; ti++)
		{
			try
			{
				...
				if (criteriaTableId > 0)
				{
					...
					String lsRowQuery  = TableViewManager.getRowQuery(connection, tableId, userId, criteriaTableId, true, "LATEST", "RESULTSET", resultList);
					resultList.forEach((x -> accessibleRowsAfterSubmit.add(x)));

					//Modified by Lakshman on 20191022 to fix the Issue Id: 17353 -- START
					String queryCheckPurgedRow = null;
					PreparedStatement psCheckPurgedRow = null;
					...
					while (rowIdsFromClientIter.hasNext())
					{
						...
						if (
							rowIdFromClient > 0 
							&& !accessibleRowsAfterSubmit.contains(rowIdFromClient) 
							&& !delRowsOnServer.contains(rowIdFromClient) //Modified by Jeetendra on 20171111 to fix Issue ID: 5120
							&& !xlDeleteRows.contains(rowIdFromClient) // deleted rows will not be fetched
						){
							if (queryCheckPurgedRow == null){
								queryCheckPurgedRow = "SELECT " + rowIdFromClient + " AS ID ";
							}
							else{
								queryCheckPurgedRow = queryCheckPurgedRow + " UNION ALL SELECT " + rowIdFromClient;
							}
						}
					}
					//System.out.println("queryCheckPurgedRow: " + queryCheckPurgedRow);

					if(queryCheckPurgedRow != null){
						queryCheckPurgedRow = 	" SELECT ID FROM (" + queryCheckPurgedRow + ") C " + 
												" WHERE EXISTS( " +
												" SELECT ID FROM BW_ROW S WITH (NOLOCK) " +
												" WHERE C.ID = S.ID " +
												" AND S.BW_TBL_ID = ? )";
						//System.out.println("queryCheckPurgedRow: " + queryCheckPurgedRow);

						psCheckPurgedRow = connection.prepareStatement(queryCheckPurgedRow);
						psCheckPurgedRow.setInt(1, tableId);
						ResultSet rsCheckPurgedRow = psCheckPurgedRow.executeQuery();

						while (rsCheckPurgedRow.next())
						{
							int xlErrorRow = rsCheckPurgedRow.getInt(1);

							System.out.println("The row id = " + xlErrorRow + " is not accessible for user");
							xlErrorCells.add(new xlErrorNew(tableId, xlErrorRow, 0, 12018));
							throw new BoardwalkException(12018, "Access Filter violation");
						}

						psCheckPurgedRow.close();
						psCheckPurgedRow = null;
						rsCheckPurgedRow.close();
						rsCheckPurgedRow = null;
					}
				//Modified by Lakshman on 20191022 to fix the Issue Id: 17353 -- END
				}
...
*/
		oldSheetCheck(connection, tableId, memberId, userId, criteriaTableId, exportTid, view);
				...
				if (colCellAccess.get(colId) instanceof String){
					...
					if(matcher.matches())
					{
						...
						if (rowSet == null)
						{
							...
							String rowQuery = TableViewManager.getRowQuery(connection, TableViewManager.getCriteriaForDynamicView(accessInstr), tableId, true, "RESULTSET", resultList);
