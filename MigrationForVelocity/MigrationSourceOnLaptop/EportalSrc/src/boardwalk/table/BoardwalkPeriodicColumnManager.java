...
//			getStatusCells
	...
	throws SQLException
	{
		...
		try
		{
			lsSql = " SELECT DISTINCT BW_TBL.ID, BW_COLUMN.ID FROM BW_TBL, BW_COLUMN WHERE BW_COLUMN.BW_TBL_ID = BW_TBL.ID AND BW_COLUMN.NAME = '"+ name +"' AND BW_COLUMN.BW_TBL_ID IN ( "+ TempStr +")";
			stmt = connection.createStatement();
			resultset = stmt.executeQuery(lsSql);
