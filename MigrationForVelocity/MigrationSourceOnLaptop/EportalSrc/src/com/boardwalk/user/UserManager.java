...
import com.boardwalk.database.DatabaseLoader;  	//Added from BLOCK-CHAIN DEMO WEB-APP
import java.util.*;
import java.io.*;
...
public class UserManager
{
	...
	private static String BW_CHECK_USER_MEMBERSHIP = "{ CALL BW_CHECK_USER_MEMBERSHIP(?,?,?,?)}";	//Added to get the Membership ID in case of Multiple Membership //	//Added from BLOCK-CHAIN DEMO WEB-APP
...
//			connection3 = databaseloader.getConnection();
			String a_table_name =	"AdminForDemo";			//"_BW_USER_ACCESS";
	...
	static public HashMap get_priviliges_for_user(String userName, String nhName)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				{
					String addnew_user = rs.getString("STRING_VALUE");
				...
				{
					String deactivate_user = rs.getString("STRING_VALUE");
				...
				{
					String add_member = rs.getString("STRING_VALUE");
				...
				{
					String delete_member = rs.getString("STRING_VALUE");
    ...
    static public Member authenticateMember(Connection a_connection, String userEmail, String password, int memberId) {
        PreparedStatement preparedstatement = null;
        ResultSet rs = null;
        int userId = -1;
        int nhId = -1;
...
////////////////////////////////////////////////////
    ...
    static public int authenticateUser(Connection a_connection, String userEmail, String password) {
        PreparedStatement preparedstatement = null;
        ResultSet rs = null;
        int userId = -1;
		String active = "";
        ...
        try {
			 ...
			 if(checkStatus == true)
			{
				String lsQuery = "select ID,ACTIVE from BW_USER where BW_USER.EMAIL_ADDRESS=? AND BW_USER.PASSWORD=? AND ACTIVE =1";
			...
			else
			{
				...
				String lsQuery = "select ID from BW_USER where BW_USER.EMAIL_ADDRESS=? AND BW_USER.PASSWORD=?";
				preparedstatement = a_connection.prepareStatement(lsQuery);
