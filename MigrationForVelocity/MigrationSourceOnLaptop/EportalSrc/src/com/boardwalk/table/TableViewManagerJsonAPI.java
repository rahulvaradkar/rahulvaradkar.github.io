 ...
 * @version
 */
public class TableViewManagerJsonAPI {
...
//					q_where = q_where + " AND CAST(CellCol" + fcColNum + ".STRING_VALUE AS decimal(38,2)) " + lsFirstChar + " " + BoardwalkUtility.replaceSQLString(lsValuePortion) + " " ;
			...
			int tableId, int accTableId, int userId) throws NumberFormatException, SQLException
	{
		...
		ResultSet rs = null;
		System.out.println("TableViewManagerJsonAPI::getColumnAccess()");
		try {
	...
	throws SQLException {
		try {
			...
			if (accessTableId > 0) {
				...
				restrColumnList = TableViewManagerJsonAPI.getRestrictedColumnsForImport(connection, tableId, accessTableId, userId);
			}
			//  JSON Changes
...
//				if (c == colv.size() - 1)
//				resJsonData.put(" ");			//COMMENTED BY RAHUL VARADKAR ON 20-FEB-2018
				//  JSON Changes
			}
			//  JSON Changes
...
//				if (r == rowv.size() - 1)
//				resJsonRowData.put(" ");			//COMMENTED BY RAHUL VARADKAR ON 20-FEB-2018
				//  JSON Changes
			}
			//  JSON Changes
			//resData.append(ContentDelimeter);
...
//			resJsonHeader.put(" ");			//COMMENTED BY RAHUL VARADKAR ON 20-FEB-2018
			...
			//--------------------------//   resjsonheader end
			//System.out.println("  TableViewManagerJsonAPI="+resHeader.toString() + ContentDelimeter + resData.toString());
			//return resHeader.toString() + ContentDelimeter + resData.toString();
			//return temptestdata.toString();		-- COMMENTED BY RAHUL VARADKAR ON 13-FEBRUARY-2018
...
//			resJsonHeader.put(" ");				//COMMENTED BY RAHUL VARADKAR ON 20-FEB-2018
			...
			//System.out.println("  TableViewManagerJsonAPI="+resHeader.toString() + ContentDelimeter + resData.toString());
			//return resHeader.toString() + ContentDelimeter + resData.toString();
			//return temptestdata;		-- ORIGINAL COMMENTED BY RAHUL VARADKAR ON 13-FEBRUARY-2018
			return responseJsonBuffer.toString();
			//  JSON Changes
		}
		catch (Exception e)
	...
	public static TableContents getFiltredTableContentsForBrowser(int tableId, int userId, int memberId, int nhId, int critTableId, int baselineId, String view , int asOfTid, long asOfDate, int compTid, long compDate)
	{
		...
		try
		{
			lsRowQuery = TableViewManagerJsonAPI.getRowQuery(connection, tableId, userId, critTableId, true, view);
...
//				System.out.println(" getTableAsOf asOfTid "+asOfTid +" asOfDate "+ asOfDate);
				...
				tbrowInfo = TableViewManagerJsonAPI.getFiltredTableRows_T(connection, tableId, userId, nhId, view, lsRowQuery, baselineId,  asOfTid, asOfDate);
...
//				System.out.println(" getTableDelta compTid "+compTid+" compDate "+ compDate);
				tbrowInfo = TableViewManagerJsonAPI.getFiltredTableRows_T(connection, tableId, userId, nhId, view, lsRowQuery, baselineId, compTid, compDate);
//				System.out.println(" getTableDelta compTid Done ");
			}
			else
			{
//				System.out.println(" Else ");
				tbrowInfo = TableViewManagerJsonAPI.getFiltredTableRows(connection, tableId, userId, lsRowQuery, baselineId);
			}
		}
		catch (Exception e)
		{
...
//		System.out.println(" compDate "+compDate);
		...
		try
		{
			lsRowQuery = TableViewManagerJsonAPI.getRowQuery(connection, tableId, userId, critTableId, false, view);
//			System.out.println(" lsRowQuerylsRowQuerylsRowQuerylsRowQuery ->"+lsRowQuery);
		}
		catch (Exception e)
		{
