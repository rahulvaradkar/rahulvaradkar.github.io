...
import servlets.xlLinkImportServiceJsonSimulator;
 ...
 * @version
 */
public class TableViewManagerJsonSimulator {
		...
		while (ri.hasNext())
		{
			...
			while (cri.hasNext())
			{
				...
				if (fc.gettrgtColumnId() > 0)
					q_from = q_from + "BW_CELL AS CellCol" + fcColNum + " ";
				else
					q_from = q_from +
...
//					q_where = q_where + " AND CAST(CellCol" + fcColNum + ".STRING_VALUE AS decimal(38,2)) " + lsFirstChar + " " + BoardwalkUtility.replaceSQLString(lsValuePortion) + " " ;
			...
			int tableId, int accTableId, int userId) throws NumberFormatException, SQLException
	{
		...
		ResultSet rs = null;
		System.out.println("TableViewManagerJsonSimulator::getColumnAccess()");
		try {
...
//					System.out.println("facl = " + facl);
	...
	//Below function modified by g2 050616
	//The original code is backedup as getTableBuffer_Old050616 in same file
	public static JSONObject getTableBuffer(Connection connection,
	int tableId,
	int userId,
	int memberId,
	int nhId,
	...
	throws SQLException {
		try {
			...
			int accessTableId = TableViewManagerJsonSimulator.getAccessTable(connection, tableId, userId);
			HashSet restrColumnList = new HashSet();
			if (accessTableId > 0) {
				System.out.println("Using access table id = " + accessTableId);
				// read the access for the user
				restrColumnList = TableViewManagerJsonSimulator.getRestrictedColumnsForImport(connection, tableId, accessTableId, userId);
			}
			//  JSON Changes
			...
			for (int c = 0; c < colv.size(); c++) {
				...
				resJsonData.put(col.getId());
				resJsonData.put(col.getName());
				resJsonData.put(col.getLookupColumnId());
				resJsonData.put(col.getLookupTableId());
				resJsonData.put(col.getAttributes());
				if (c == colv.size() - 1)
				resJsonData.put(" ");
				//  JSON Changes
			}
			...
			for (int r = 0; r < rowv.size(); r++) {
				...
				resJsonRowData.put(rowObject.getId());
				if (r == rowv.size() - 1)
				resJsonRowData.put(" ");
				//  JSON Changes
			}
			//  JSON Changes
			//resData.append(ContentDelimeter);
			JSONObject objCellData = new JSONObject();
			//  JSON Changes
				...
				if (results) {
					...
					} else // cell data
					{
						...
						for (int c = 0; c < colv.size(); c++) {
							...
							JSONArray resJsonFmlData = new JSONArray();
							//  JSON Changes
							xlColumn_import col = (xlColumn_import) colv.elementAt(c);
							...
							for (int r = 0; r < rowv.size(); r++) {
								...
								if (r == rowv.size() - 1) // last cell of the column
								{
									...
									resJsonCellData.put(cellval);
									resJsonFmlData.put(cellFormula);
									//  JSON Changes
								} else {
									//  JSON Changes
							...
							objCellData.put("col" + c, resJsonCellData);
							objCellData.put("colfml" + c, resJsonFmlData);
			...
			resJsonHeader.put("Success");
			resJsonHeader.put(tableId);
			resJsonHeader.put(tinfo.getTableName());
			resJsonHeader.put(tinfo.getTablePurpose());
			resJsonHeader.put(view);
			resJsonHeader.put(userId);
			resJsonHeader.put(memberId);
			resJsonHeader.put(nhId);
			resJsonHeader.put(colv.size() - restrColumnList.size());
			resJsonHeader.put(rowv.size());
			resJsonHeader.put(maxTransactionId);
			resJsonHeader.put(exportTid);
			resJsonHeader.put(criteriaTableId);
			resJsonHeader.put(mode);
			resJsonHeader.put(" ");
			...
			//System.out.println("objCellData=" + objCellData.toString());

			JSONObject responseJsonBuffer = new JSONObject();
			responseJsonBuffer.put("header", resJsonHeader);
			responseJsonBuffer.put("resJsonData", resJsonData);
			responseJsonBuffer.put("resJsonRowData", resJsonRowData);
			responseJsonBuffer.put("objCellData", objCellData);

			//System.out.println("final json obj" + responseJsonBuffer.toString());
			String[] jskeys = {
				"header",
				"resJsonData",
				"resJsonRowData",
...
/*			xlLinkImportServiceJsonSimulator s = new xlLinkImportServiceJsonSimulator();
			
			//String temptestdata="";
			StringBuffer temptestdata = new StringBuffer();
			for (int js = 0; js < responseJsonBuffer.length(); js++) {
				JSONArray testa = new JSONArray();
				if (jskeys[js] != "objCellData") {
					testa = (JSONArray) responseJsonBuffer.get(jskeys[js]);

					for (int jsa = 0; jsa < testa.length(); jsa++) {
						//String spaceelim=(String)testa.get(jsa);
						//System.out.println("values="+testa.get(jsa));
						if (testa.get(jsa) != " " && jsa != testa.length() - 1)
							//temptestdata = temptestdata + testa.get(jsa) + Seperator;
							temptestdata.append(testa.get(jsa) + Seperator);
						else if (jsa == testa.length() - 1) {
							if (testa.get(jsa) != " ")
								//temptestdata = temptestdata;
							//else
								//temptestdata = temptestdata + testa.get(jsa);
								temptestdata.append(testa.get(jsa));
						} else if (testa.get(jsa) == " ")
					...
					//String[] jskeys1={"col0","colfml0","col1","colfml1"};
					int c = responseJsonBuffer.getJSONObject("objCellData").length();
					int temp = 0;
					while (c > 0)
					//for(int testobj=0;testobj<responseJsonBuffer.getJSONObject("objCellData").length();testobj++)
					{
						System.out.println("c=" + c + " temp=" + temp);

						JSONArray testcellarr = new JSONArray();
						JSONArray testfmlarr = new JSONArray();
						testcellarr = (JSONArray) responseJsonBuffer.getJSONObject("objCellData").getJSONArray("col" + temp);
						testfmlarr = (JSONArray) responseJsonBuffer.getJSONObject("objCellData").getJSONArray("colfml" + temp);
						//responseJsonBuffer.getJSONObject("t2").getJSONArray(jskeys1[js]);
						for (int jsa = 0; jsa < testcellarr.length(); jsa++) {
							//String spaceelim=(String)testcellarr.get(jsa);
							//System.out.println("values="+testcellarr.get(jsa));
							if (testcellarr.get(jsa) != " ")
							if (jsa != testcellarr.length() - 1)
								//temptestdata = temptestdata + testcellarr.get(jsa) + Seperator;
								temptestdata.append(testcellarr.get(jsa) + Seperator);
							else if (jsa == testcellarr.length() - 1) {
								if (testcellarr.get(jsa) != " ")
									//temptestdata = temptestdata;
								//else
									//temptestdata = temptestdata + testcellarr.get(jsa);
									temptestdata.append( testcellarr.get(jsa));
							} else if (testcellarr.get(jsa) == " ")
								//temptestdata = temptestdata + Seperator;
								temptestdata.append(Seperator);
						}
						//temptestdata = temptestdata + ContentDelimeter;
						temptestdata.append(ContentDelimeter);
						for (int jsa = 0; jsa < testfmlarr.length(); jsa++) {
							//String spaceelim=(String)testcellarr.get(jsa);
							//System.out.println("values="+testcellarr.get(jsa));
							if (testfmlarr.get(jsa) != " ")
							if (jsa != testfmlarr.length() - 1)
								//temptestdata = temptestdata + testfmlarr.get(jsa) + Seperator;
								temptestdata.append(testfmlarr.get(jsa) + Seperator);
							else if (jsa == testfmlarr.length() - 1) {
								if (testfmlarr.get(jsa) != " ")
									//temptestdata = temptestdata;
								//else
									//temptestdata = temptestdata + testfmlarr.get(jsa);
									temptestdata.append( testfmlarr.get(jsa));
							} else if (testfmlarr.get(jsa) == " ")
								//temptestdata = temptestdata + Seperator;
								temptestdata.append(Seperator);
						}

						//System.out.println("objcelldata testcellarr"+temptestdata);
						//temptestdata = temptestdata + ContentDelimeter;
						temptestdata.append(ContentDelimeter);
			...
			File fileDir = new File("d:/tomcat7/webapps/EPortal/Staging_JSON/Tableviewlinkimport.json");
			Writer out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileDir), "UTF8"));
			out.append(responseJsonBuffer.toString());
			out.flush();
			out.close();
			...
			//--------------------------//   resjsonheader end
			//System.out.println("  TableViewManagerJsonSimulator="+resHeader.toString() + ContentDelimeter + resData.toString());
			//return resHeader.toString() + ContentDelimeter + resData.toString();
			return temptestdata.toString();
			//  JSON Changes
*/
		return responseJsonBuffer;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	public static String getTableBuffer_Old050616(Connection connection,
												int tableId,
												int userId,
												int memberId,
												int nhId,
		...
		throws SQLException
	{
		try
		{
			...
			System.out.println("final json obj"+responseJsonBuffer.toString());
			String[] jskeys={"header","resJsonData","resJsonRowData","objCellData"};
			String temptestdata="";
			for(int js=0;js<responseJsonBuffer.length();js++)
			{
			JSONArray testa=new JSONArray();
			...
			temptestdata=temptestdata+ContentDelimeter;
			 ...
			 else
				{
								...
								//for(int testobj=0;testobj<responseJsonBuffer.getJSONObject("objCellData").length();testobj++)
							{
								...
								//String spaceelim=(String)testcellarr.get(jsa);
								//System.out.println("values="+testcellarr.get(jsa));
								if(testcellarr.get(jsa)!=" ")if( jsa!=testcellarr.length()-1)
									temptestdata=temptestdata+testcellarr.get(jsa)+Seperator;
								else if(jsa==testcellarr.length()-1)
								{
								if(testcellarr.get(jsa)==" ")
								...
								//String spaceelim=(String)testcellarr.get(jsa);
								//System.out.println("values="+testcellarr.get(jsa));
								if(testfmlarr.get(jsa)!=" ") if ( jsa!=testfmlarr.length()-1)
									temptestdata=temptestdata+testfmlarr.get(jsa)+Seperator;
								else if(jsa==testfmlarr.length()-1)
								{
								if(testfmlarr.get(jsa)==" ")
			...
			System.out.println("   temptestdata="+temptestdata);
			File fileDir = new File("d:/tomcat7/webapps/EPortal/Staging_JSON/Tableviewlinkimport.json");
					 Writer out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileDir), "UTF8"));
		         out.append(responseJsonBuffer.toString());
				 out.flush();
	...
	public static TableContents getFiltredTableContentsForBrowser(int tableId, int userId, int memberId, int nhId, int critTableId, int baselineId, String view , int asOfTid, long asOfDate, int compTid, long compDate)
	{
		...
		try
		{
			lsRowQuery = TableViewManagerJsonSimulator.getRowQuery(connection, tableId, userId, critTableId, true, view);
...
//				System.out.println(" getTableAsOf asOfTid "+asOfTid +" asOfDate "+ asOfDate);
				...
				tbrowInfo = TableViewManagerJsonSimulator.getFiltredTableRows_T(connection, tableId, userId, nhId, view, lsRowQuery, baselineId,  asOfTid, asOfDate);
...
//				System.out.println(" getTableDelta compTid "+compTid+" compDate "+ compDate);
				tbrowInfo = TableViewManagerJsonSimulator.getFiltredTableRows_T(connection, tableId, userId, nhId, view, lsRowQuery, baselineId, compTid, compDate);
//				System.out.println(" getTableDelta compTid Done ");
			}
			else
			{
//				System.out.println(" Else ");
				tbrowInfo = TableViewManagerJsonSimulator.getFiltredTableRows(connection, tableId, userId, lsRowQuery, baselineId);
			}
		}
		catch (Exception e)
		{
...
//		System.out.println(" compDate "+compDate);
		...
		try
		{
			lsRowQuery = TableViewManagerJsonSimulator.getRowQuery(connection, tableId, userId, critTableId, false, view);
//			System.out.println(" lsRowQuerylsRowQuerylsRowQuerylsRowQuery ->"+lsRowQuery);
		}
		catch (Exception e)
		{
