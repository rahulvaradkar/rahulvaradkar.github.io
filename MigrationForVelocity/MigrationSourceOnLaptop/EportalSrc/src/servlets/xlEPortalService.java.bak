package servlets;
/*
 *  xlEPortalService.java
 */

import boardwalk.eportal.AdminTasks;
import java.io.BufferedReader;

//  following imports are COPIED TO boardwalk.eportal.AdminTask
...
import org.json.JSONArray;				//Added by Rahul Varadkar n 11-MARCH-2018 for Link Export Table
...
//Added by Rahul Varadkar on 11-MARCH-2018 for Link Export Table
...
public class xlEPortalService extends xlService implements SingleThreadModel
{
...
//		Reading JSON data from Request object
...
//			String role = jObj.getString("role");
	...
	//		int action;
			...
			String wrkstr;
			wrkstr = "test"  ; // st.nextToken(Seperator);
	...
	// Refresh Tree Node in BCP Mgmt studio		71
			...
			if (bwcon != null)
			{
				System.out.println("Successfully obtained authenticated Boardwalk Connection in xlEPortalService");
				System.out.println("xlEPortalService inside bwcon ...Action=" + action);
	...
	//						collabId = CreateCollaboration (bwcon, collabName, desc);
							collabId = AdminTasks.CreateCollaboration(connection, collabName, desc, userId, memberId);
							if (collabId != -1)
						...
						case 2:						//RenameCollaboration"
							...
							collabId = AdminTasks.RenameCollaboration (connection, collabId, newCollabName);
							if (collabId != -1)
						...
						case 3:						//DeleteCollaboration
							...
							if (AdminTasks.DeleteCollaboration (bwcon, collabId))
						...
						case 4:						//GetCollaborationTreeUsingId
							...
							collabId = Integer.parseInt(wrkstr);
							String collabTree = "";
							collabTree = AdminTasks.GetCollaborationTreeUsingId(bwcon, collabId);
						...
						case 6:						//CreateWhiteboard
							...
							wrkstr = st.nextToken(Seperator);
							wbName = wrkstr;
							wbId = AdminTasks.CreateWhiteboard (bwcon, collabId, wbName);
							if (wbId != -1)
						...
						case 7:						//RenameWhiteBoard
							...
							wbId = AdminTasks.RenameWhiteboard(connection, wbId, newWbName);
							if (wbId != -1)
						...
						case 8:						//DeleteWhiteboard
							...
							wrkstr = st.nextToken(Seperator);
							wbId = Integer.parseInt(wrkstr);
							if (AdminTasks.DeleteWhiteboard(connection, wbId))
						...
						case 9:						//CreateTable
							...
							wrkstr = st.nextToken(Seperator);
							desc = wrkstr;
							tableId = AdminTasks.CreateTable (bwcon, collabId, wbId, tableName, desc);
							if (tableId != -1)
						...
						case 10:					//DeleteTable
							...
							if (AdminTasks.DeleteTable(bwcon, tableId))
						...
						case 14:					//CreateNeighborhood
							...
							nhId = AdminTasks.CreateNeighborhood(bwcon, nhName, parentNhId);
						...
						case 15:					//DeleteNeighborhood
							...
							wrkstr = st.nextToken(Seperator);
							nhName = wrkstr;
							if (AdminTasks.DeleteNeighborhood(bwcon, nhId))
						...
						case 17:					//CreateMembershipForNeighborhood
							...
							memberId = AdminTasks.CreateMembershipForNeighborhood(bwcon, nhId, userId);
						...
						case 18:					//DeleteMembership
							wrkstr = st.nextToken(Seperator);
							memberId = Integer.parseInt(wrkstr);
							AdminTasks.DeleteMembership(bwcon, memberId);
	...
	*/
						...
						case 22:					//CreateNewUser
							...
							pwd = st.nextToken(Seperator);
							String firstname = st.nextToken(Seperator);
							String lastname = st.nextToken(Seperator);
							
							userId = AdminTasks.CreateNewUser(bwcon, user, pwd, firstname, lastname);
						...
						case 23:					//GetUserList
							String userList = "";
							userList = AdminTasks.GetUserList(bwcon);
							//responseBuffer = "SUCCESS" + Seperator +  "The Neighborhood of " + nhId + "is \n" + tree + ContentDelimeter + nhId  ;
							...
							jsonReturn.accumulate("Elements", ulist);
						...
						case 28:					//RenameTable
							...
							tableId = AdminTasks.RenameTable(connection, tableId, newTableName);
							if (tableId != -1)
						...
						case 29:					//RenameNh
							...
							nhId = AdminTasks.RenameNh(connection, nhId, newNhName);
							if (nhId != -1)
						...
						case 30:
							...
							try
							{
								allMembershipInfo = AdminTasks.GetAllMembershipsInfo(connection);
						...
						case 31:
							...
							try
							{
								AllNhCollabTree = AdminTasks.GetCollaborationTreeForAllNh(connection, bwcon);
						...
						case 32:			//Get Collaboration Tree For Nh ManagedBy User
							...
							try
							{
								NhCollabTreeForUser = AdminTasks.GetCollaborationTreeForNhManagedByUser(connection, bwcon, userId);
						...
						case 34:			//GetAllNeighborhoodTree		
							...
							nhId = Integer.parseInt(wrkstr);
							String nhAlltree = "";
							nhAlltree = AdminTasks.GetAllNeighborhoodTree(connection, nhId);
							//responseBuffer = "SUCCESS" + Seperator +  "The Neighborhood of " + nhId + "is \n" + tree + ContentDelimeter + nhId  ;
						...
						case 35:			//GetColumnAccessOnTable()
							...
							//{
								...
								columnAccess = AdminTasks.getColumnAccessForUpdate(bwcon, connection, tableId);
						...
						case 36:			//SetColumnAccessOnTable()
							...
							try
							{
								...
								AdminTasks.SetColumnAccessOnTable(bwcon, connection, columnAccess);					// original
								responseBuffer = "SUCCESS" + ContentDelimeter +  columnAccess + ContentDelimeter + tableId ;
							}
							catch (Exception e)
							{
						...
						case 37:		//GetCuboidColumnAccessForAllCuboids
							...
							try
							{
								//columnAccess = GetColumnAccessOnTable(connection, tableId);
								columnAccess = "";
								columnAccess = AdminTasks.GetCuboidColumnAccessForAllCuboids(connection);					// original
						...
						case 38:		//GetAllColumnAccessForCuboid
							...
							try
							{
								//columnAccess = GetColumnAccessOnTable(connection, tableId);
								columnAccess = "";
								columnAccess = AdminTasks.GetAllColumnAccessForCuboid(connection, tableId);					// original
								// getColumnAccessForUpdate(connection, 
								//columnAccess = SetColumnAccessOnTable(connection, columnAccess,userId);		// by Arun
								//responseBuffer = "SUCCESS" + Seperator +  "The Neighborhood of " + nhId + "is \n" + tree + ContentDelimeter + nhId  ;
								responseBuffer = "SUCCESS" + ContentDelimeter +  columnAccess + ContentDelimeter ;
							   ...
							   Vector nhRel = null;
								...
								if ( nhId != -1 )
								{
								   ...
								   Vector nhTree = NeighborhoodManager.getNeighborhoodTree(connection, userId);
									...
									str = AdminTasks.processRelation( nm, nhRel, nhTree);
						...
						case 40:		//AddNewRelation()
							...
							AdminTasks.AddNewRelation(connection, nhId, userId, relation, selNhIds);
						...
						case 41:		//UpdateRelation()
							...
							if(AdminTasks.UpdateRelation(connection, nhId, userId, relation, selNhIds))
							{
						...
						case 43:		//GetBRectInformation
							...
							{
								String strBRectInfo = "";
								strBRectInfo = AdminTasks.GetBRectInformation(bwcon);
						...
						case 44:		//UpdateBRectDefinition()
							boolean bln = AdminTasks.UpdateBRectDefinition(bwcon);
							if (bln)
							{
						...
						case 45:		//Get User Roles and Role Users Mappings()
							...
							{
								String strRoles = "";
								strRoles = AdminTasks.GetRolesFromRoleMaster(bwcon);

								String strRoleUsers = "";
								strRoleUsers = AdminTasks.GetDefinedRolesUsers(bwcon);
						...
						case 46:		//Update User Roles
							...
							try
							{
								AdminTasks.UpdateUserRoleChanges(bwcon, wrkstr);
						...
						case 47:		//Get Template Manifest Details
							try
							{
								...
								strTemplateManifestBrects = AdminTasks.GetTemplateManifestsFromBRectDefinition(bwcon);
						...
						case 48:		//Get BRect Types
							try
							{
								...
								strBRectTypes = AdminTasks.getBRectTypes(bwcon);
						...
						case 49:		//Get BRect Definitions
							try
							{
								...
								strBRectNames = AdminTasks.getBRectNamesOfType(bwcon, wrkstr);
						...
						case 50:		//Get BRectNames defined in BRect Definitions
							try
							{
								...
								strBRectDetails = AdminTasks.getBRectDetails(bwcon, wrkstr);
						...
						case 51:		//Get KeyName Details defined in Key Store
							try
							{
								...
								strKeyStoreDetails = AdminTasks.getKeyStoreDetails(bwcon, wrkstr);
						...
						case 52:		//Get System Cuboid list
							try
							{
								...
								strSystemCuboidDetails = AdminTasks.getSystemCuboidsDetails(bwcon);
						...
						case 53:		//Get Columns in BRect
							try
							{
								...
								strBRectColumns = AdminTasks.getBRectColumns(bwcon, wrkstr);
						...
						case 54:		//GENERATE_SQL_BRECT FROM BRECT NAME
							try
							{
								...
								strSQLBrectName = AdminTasks.generateSQLBrect(connection, strBRectName, strSQLBrectName);
						...
						case 55:				//Get Supermerge User Access details 
							try
							{
								...
								strSMRuleNames = AdminTasks.GetSuperMergeRuleNames(bwcon);
								System.out.println("strSMRuleNames = " + strSMRuleNames);

								String strRuleUserAccess = "";
								strRuleUserAccess = AdminTasks.GetSuperMergeRuleUserAccess(bwcon);
						...
						case 56:				// Update SuperMerge User Access Settings
							...
							try
							{
								AdminTasks.UpdateSuperMergeUserAccessChanges(bwcon, wrkstr);
								responseBuffer = "SUCCESS" + ContentDelimeter +  wrkstr + ContentDelimeter  ;
							}
							catch (Exception e)
							{
						...
						case 57:				// Run Cuboid to SQL SuperMerge Rule
							...
							try
							{
								AdminTasks.RunCuboidToSQLSuperMergeRule(bwcon, smRuleBRectName, smRuleName, UserName, targetSQLTable );
								responseBuffer = "SUCCESS" + ContentDelimeter +  wrkstr + ContentDelimeter  ;
							}
							catch (Exception e)
							{
						...
						case 58:				// Link Import B-Rect Resultset
							...
							try
							{
								bRectBuffer = AdminTasks.GetBRectResultSet(connection, BRectName);
						...
						case 59:				// Truncate SQL Table
							...
							try
							{
								AdminTasks.TruncateSQLTable(connection, sqlTableName);
						...
						case 60:				// Get Cuboid Id
							...
							{
								int CuboidId ;
								CuboidId = AdminTasks.GetCuboidId(connection, strBRectName);	
						...
						case 61:				// Get Collaboration Tree created by user
							...
							try
							{
								collabTree = AdminTasks.GetCollaborationTreeCreatedbyUser(connection, bwcon, userId);	
						...
						case 62:				// Get Columns of Source and Target
							...
							try
							{
								sourceTargetColumns = AdminTasks.GetColumnsOfSourceAndTarget(connection, bwcon, sourceTableId, targetTableId);
						...
						case 63:				// Get Enterprisewise NH Hierarchy using User
							...
							userId = Integer.parseInt(wrkstr);
							String EntTree = "";
							EntTree = AdminTasks.GetEnterproseWideNeighborhoodTree(bwcon, connection, userId);
							//responseBuffer = "SUCCESS" + Seperator +  "The Neighborhood of " + nhId + "is \n" + tree + ContentDelimeter + nhId  ;
						...
						case 64:				// Get NH Hierarchy Tree for Neighborhood Administrator 
							...
							System.out.println("64 : userName = " + userName);
							String NhAdminTree = "";
							try
							{
								...
								if (NhAdminTree.toUpperCase().startsWith("FAILURE"))
								{

									String msg = NhAdminTree.replace("FAILURE" + ContentDelimeter, "");
									map = new HashMap<String, String>();
						...
						case 65:				// Check if user is Neighborhood Administrator 
							...
							try
							{
								isNhAdmin = AdminTasks.IsUserNhAdministrator( connection, userName);
						...
						case 66:				// Check if user is Super User 
							...
							try
							{
								isSuperUser = AdminTasks.IsUserSuperUser( connection, userName);
						...
						case 67:				// Check if user can LINK IMPORT
							...
							try
							{
								canLinkImport = AdminTasks.CanUserLinkImport( connection, userName);
						...
						case 68:				// Check if user can LINK EXPORT
							...
							try
							{
								canLinkExport = AdminTasks.CanUserLinkExport( connection, userName);
						...
						case 69:				// Get __COLLAB_STRUCTRUE Cuboid data
							...
							try
							{
								...
								strCollabInfo = AdminTasks.Get__CollabStructure(bwcon);
						...
						case 70:				// Update __COLLAB_STRUCTRUE Cuboid data
							boolean blnn = AdminTasks.Update__COLLAB_STRUCTURE(bwcon);
							if (blnn)
							{
						...
						case 74:				// GetSelectFilterCuboidView
							System.out.println("case is 74");

							String strServerUrl_1 = jObj.getString("serverUrl");
							String strCuboidId_1 = jObj.getString("cuboidId");
							String strColumnName_1 = jObj.getString("columnName");
							String filterCondition = jObj.getString("filter");
							String OrderbyClause = jObj.getString("orderBy");
							String columnDataTypes = jObj.getString("dataType");

							System.out.println("filterCondition : " + filterCondition);
							System.out.println("strServerUrl : " + strServerUrl_1);
							System.out.println("OrderbyClause : " + OrderbyClause);
							System.out.println("strCuboidId : " + strCuboidId_1);
							System.out.println("strColumnName : " + strColumnName_1);

							String strSequence_1 = jObj.getString("sequence");
							System.out.println("strSequence_1 : " + strSequence_1);

							String receivedTokens_[] ;
							receivedTokens_	= strColumnName_1.split(",");
							int noCols_1 = receivedTokens_.length + 1;			//1 is added for RowId

							String columnData_1 = null;

							columnData_1 = AdminTasks.GetSelectFilterCuboidView(connection, Integer.parseInt(strCuboidId_1), strColumnName_1, filterCondition, OrderbyClause) ;
							...
							List<String> colDataList_1 = new ArrayList<String>();
							colDataList_1.add(strSequence_1);
							colDataList_1.add(columnData_1);
									
							jsonReturn.accumulate("Elements", colDataList_1);
						//	System.out.println(jsonReturn.toString());

							break;


						case 73:				// Get Column Data
							System.out.println("case is 73");

						//	String strNodeType1 = jObj.getString("nodeType");
							String strServerUrl1 = jObj.getString("serverUrl");
						//	String strCuboidName1 = jObj.getString("cuboidName");
							String strCuboidId1 = jObj.getString("cuboidId");
							String strColumnName1 = jObj.getString("columnName");
						//	String strColumnId1 = jObj.getString("columnId");

						//	System.out.println("strNodeType : " + strNodeType1);
							System.out.println("strServerUrl : " + strServerUrl1);

						//	System.out.println("strCuboidName : " + strCuboidName1);
							System.out.println("strCuboidId : " + strCuboidId1);
							System.out.println("strColumnName : " + strColumnName1);
					//		System.out.println("strColumnId : " + strColumnId1);

							String strSequence = jObj.getString("sequence");
							System.out.println("strSequence : " + strSequence);

							String receivedTokens[] ;
							receivedTokens	= strColumnName1.split(",");
							int noCols = receivedTokens.length + 1;			//1 is added for RowId

							String columnData = null;

							columnData = AdminTasks.GetSelectColumnList(connection, Integer.parseInt(strCuboidId1), strColumnName1) ;
							...
							colDataList.add(strSequence);
							colDataList.add(columnData);
									
							jsonReturn.accumulate("Elements", colDataList);
						//	System.out.println(jsonReturn.toString());

							break;

						case 71:				// Rrefresh Node in BCP Management Studio

							System.out.println("case is 71");
//{'action':'64', 'serverUrl':'http://localhost:8080/EPortal', 'serverName':'EPortal', 'serviceName':'xlEPortalService', 'nodePathId':'2~13', 'nodeParentId':'2', 'nodeId':'13', 'nodeLevel':'1', 'nodeType':'nh'}
//{'action':'64', 'serverUrl':'http://localhost:8080/EPortal', 'serverName':'eportal', 'serviceName':'xlEPortalService', 'nodePathId':'2~3~9', 'nodeParentId':'3', 'nodeId':'9', 'nodeLevel':'2', 'nodeType':'nh'}
//{'action':'64', 'serverUrl':'http://localhost:8080/EPortal', 'serverName':'eportal', 'serviceName':'xlEPortalService', 'nodePathId':'2~3~12~103', 'nodeParentId':'12', 'nodeId':'103', 'nodeLevel':'3', 'nodeType':'nh'}
//{'action':'64', 'serverUrl':'http://localhost:8080/EPortal', 'serverName':'eportal', 'serviceName':'xlEPortalService', 'nodePathId':'2~3~12~103', 'nodeParentId':'12', 'nodeId':'103', 'nodeLevel':'3', 'nodeType':'collabs'}
//{'action':'64', 'serverUrl':'http://localhost:8080/EPortal', 'serverName':'eportal', 'serviceName':'xlEPortalService', 'nodePathId':'2~3~12~103', 'nodeParentId':'12', 'nodeId':'103', 'nodeLevel':'3', 'nodeType':'members'}
// {"action":"71","serverUrl":"http://localhost:8080/EPortal","serverName":"epo","serviceName":"xlEPortalService","refresh_action":"23","nodeType":"users"}


//							int intParentNodeId = jObj.getInt("nodeParentId");
//							int intNodeId = jObj.getInt("nodeId");
							String strNodeType = jObj.getString("nodeType");
							String strServerUrl = jObj.getString("serverUrl");
							System.out.println("strNodeType : " + strNodeType);
							System.out.println("strServerUrl : " + strServerUrl);

							switch (strNodeType)
							{
								case "members" :
									nhId = jObj.getInt("nodeId");
									System.out.println("nhId : " + nhId);

									String memberList = "";

									memberList = AdminTasks.GetNeighborhoodMemberList(bwcon, nhId) ;
									...
									mbrlist.add(memberList);
											
									jsonReturn.accumulate("Elements", mbrlist);
									System.out.println(jsonReturn.toString());
									break;
								...
								case "collabs" :
									...
									String collabTreeList = "";

									collabTreeList = AdminTasks.GetCollaborationTreeForNh(connection, bwcon, nhId) ;
									System.out.println(collabTreeList);
									...
									ctlist.add(collabTreeList);
											
									jsonReturn.accumulate("Elements", ctlist);
									System.out.println(jsonReturn.toString());
								...
								case "nh" :
									...
									String nhTreeListXyz = "";
									nhTreeListXyz = AdminTasks.GetNeighborhoodTree(bwcon, nhId);
									System.out.println(nhTreeListXyz);

									map = new HashMap<String, String>();
									map.put("Status", "Success");
									map.put("FailureReason", "None");
									map.put("DataElements", "3");

									jsonReturn = new JSONObject();
									jsonReturn.accumulate("Header", map);

									List<String> nhNodelist = new ArrayList<String>();
									nhNodelist.add(memberList);
									nhNodelist.add(collabTreeList);
									nhNodelist.add(nhTreeListXyz);
										
									jsonReturn.accumulate("Elements", nhNodelist);
									System.out.println(jsonReturn.toString());
									break;
								...
								case "users" :
									...
									String userList1 = "";
									userList1 = AdminTasks.GetUserList(bwcon);
									System.out.println(userList1);
									//responseBuffer = "SUCCESS" + Seperator +  "The Neighborhood of " + nhId + "is \n" + tree + ContentDelimeter + nhId  ;
									responseBuffer = "SUCCESS" + Seperator +  userList1   ;
									...
									usrlist.add(userList1);
											
									jsonReturn.accumulate("Elements", usrlist);
									System.out.println(jsonReturn.toString());
								...
								case "columns" :
									cuboidId = jObj.getInt("cuboidId");
									String columnsList = "";
									columnsList = AdminTasks.GetColumnList(bwcon, cuboidId);
									...
									colList.add(columnsList);
											
									jsonReturn.accumulate("Elements", colList);
									System.out.println(jsonReturn.toString());
						...
						case 72:				// Get NH Hierarchy Tree for Neighborhood Administrator 
							String strTblWiseCells = "";
							
								strTblWiseCells = AdminTasks.GetTableWiseCellsReport(connection);
								...
								List<String> tblWsRept = new ArrayList<String>();
								tblWsRept.add(strTblWiseCells);
										
								jsonReturn.accumulate("Elements", tblWsRept);
								System.out.println(jsonReturn.toString());
...
/*							catch (Exception e)
							{
								...
								map.put("FailureReason", "Could not generate Tablewise Cell Report on Server.");
								map.put("DataElements", "1");

								jsonReturn = new JSONObject();
								jsonReturn.accumulate("Header", map);

								String ErrMsg = "FAILURE" + ContentDelimeter +  "Could not generate Tablewise Cell Report on Server.  Please contact Boardwalk Administrator."  + ContentDelimeter   ;

								List<String> tblWsRept = new ArrayList<String>();
								tblWsRept.add(ErrMsg);
...
*/
						...
						case 75:				// Link Import Table
							System.out.println("case is 75 .........Link Import Table");
							userName = jObj.getString("userName");
							pwd = jObj.getString("pwd");
							nhHierarchy = jObj.getString("nhHierarchy");
							cuboidId = jObj.getInt("cuboidId");
							System.out.println("75 : userName = " + userName);

							String linkImportBuffer = AdminTasks.LinkImportTable(connection, userName, pwd, nhHierarchy, cuboidId);

							if (linkImportBuffer.toUpperCase().startsWith("FAILURE"))
							{
								String msg = linkImportBuffer.replace("FAILURE" + ContentDelimeter, "");
								map = new HashMap<String, String>();
								map.put("Status", "Failed");
								map.put("FailureReason", msg);
								map.put("DataElements", "0");
							...
							else
							{
								...
								List<String> liBuffer = new ArrayList<String>();
								liBuffer.add(linkImportBuffer);
										
								jsonReturn.accumulate("Elements", liBuffer);
								System.out.println(jsonReturn.toString());
							}
						...
						case 76:				// Refresh Table
							System.out.println("case is 76 .........Refresh Table");

//m_userId ,m_userName ,m_userPassword ,memberId ,m_nhId ,TableId ,-1 ,view, importTid, exportTid ,mode ,synch 

							userName = jObj.getString("userName");
							pwd = jObj.getString("pwd");
							int userId = Integer.parseInt(jObj.getString("userId"));
							//String userPassword = requestInfo[2];
							memberId = Integer.parseInt(jObj.getString("memberId"));				// delete this
							nhHierarchy = jObj.getString("nhHierarchy");
							nhId = Integer.parseInt(jObj.getString("nhId"));
							tableId = Integer.parseInt(jObj.getString("cuboidId"));
							int baselineId = Integer.parseInt(jObj.getString("baselineId"));
							String view = jObj.getString("view");
							int importTid = jObj.getInt("importTId");
							int exportTid = jObj.getInt("exportTId");
							int mode = Integer.parseInt(jObj.getString("Mode"));
							int synch = Integer.parseInt(jObj.getString("synch"));
							String cuboidRows = jObj.getString("cuboidRows");

							System.out.println("76 : userName = " + userName);
							System.out.println("76 : memberId = " + memberId);
							System.out.println("76 : nhId = " + nhId);
							System.out.println("76 : tableId = " + tableId);
							System.out.println("76 : exportTid = " + exportTid);
							System.out.println("76 : importTid = " + importTid);
							System.out.println("76 : cuboidRows = " + cuboidRows);

							String refreshBuffer = "" ;
							refreshBuffer = AdminTasks.refreshTable(connection, userId, userName, pwd, memberId, nhId, tableId, baselineId, view, importTid, exportTid, mode, synch, cuboidRows);

							if (refreshBuffer.toUpperCase().startsWith("FAILURE"))
							{
								String msg = refreshBuffer.replace("FAILURE" + ContentDelimeter, "");
								map = new HashMap<String, String>();
								map.put("Status", "Failed");
								map.put("FailureReason", msg);
								map.put("DataElements", "0");
							...
							else
							{
								...
								List<String> reBuffer = new ArrayList<String>();
								reBuffer.add(refreshBuffer);
										
								jsonReturn.accumulate("Elements", reBuffer);
								System.out.println(jsonReturn.toString());
							}
						...
						case 79:				// Submit Cuboid..added by Rahul Varadkar on 20-MARCH-2018
							System.out.println("case is 79 .........Submit Cuboid...");
// VBA CODE
/*
    header = m_userId | m_userName | m_userPassword |  memberId |  m_nhId | lngTableID | view | colCount - 1 |  Rows.count - 1 |  lngImportTid | lngExportTid |  lngCritical |  criticalLevel |  comments & VBA.Chr(2)
    reqStr = header & colStr & VBA.Chr(2) & rowStr & VBA.Chr(2)
  */
							userName = jObj.getString("userName");
							pwd = jObj.getString("pwd");
							...
							nhId = Integer.parseInt(jObj.getString("nhId"));
							tableId = Integer.parseInt(jObj.getString("cuboidId"));
							baselineId = Integer.parseInt(jObj.getString("baselineId"));
							view = jObj.getString("view");
							importTid = jObj.getInt("importTId");
							exportTid = jObj.getInt("exportTId");
							int criticalLevel = Integer.parseInt(jObj.getString("criticalLevel"));
							int lngCritical = Integer.parseInt(jObj.getString("critical"));
							mode = Integer.parseInt(jObj.getString("Mode"));		
							synch = Integer.parseInt(jObj.getString("synch"));							
							String strComment = jObj.getString("comment");
							int rowCount = jObj.getInt("rowCount");
							int colCount = jObj.getInt("colCount");

							//Reading cuboidRows
							JSONArray cuboidRowsArr = jObj.getJSONArray("cuboidRows");
							String[] strCuboidRows = new String[cuboidRowsArr.length()];
							for(int i = 0; i < cuboidRowsArr.length(); i++) {
								strCuboidRows[i] = cuboidRowsArr.getString(i);
							}
							System.out.println("79 : Array intCuboidRows = " + Arrays.toString(strCuboidRows));

							//Reading cuboidCols
							JSONArray cuboidColsArr = jObj.getJSONArray("cuboidCols");
							String[] strCuboidCols = new String[cuboidColsArr.length()];
							for(int i = 0; i < cuboidColsArr.length(); i++) {
								strCuboidCols[i] = cuboidColsArr.getString(i);
							}
							System.out.println("79 : Array intCuboidCols = " + Arrays.toString(strCuboidCols));
							//String cuboidCols = jObj.getString("cuboidCols");

							//Reading cuboidCells
							JSONArray cuboidCellDataArr = jObj.getJSONArray("cuboidCells");
							System.out.println("79 : Array cuboidCellDataArr.toString= " + cuboidCellDataArr.toString());
							String jsonCuboidCellDataArr = cuboidCellDataArr.toString();

							//Reading cuboidColNames
							JSONArray cuboidColNamesArr = jObj.getJSONArray("cuboidColNames");
							String[] strCuboidColNames = new String[cuboidColNamesArr.length()];
							for(int i = 0; i < cuboidColNamesArr.length(); i++) {
								strCuboidColNames[i] = cuboidColNamesArr.getString(i);
							}
							System.out.println("79 : Array strCuboidColNames = " + Arrays.toString(strCuboidColNames));
							//String cuboidColNames = jObj.getString("cuboidColNames");


							cuboidRows = "";

							System.out.println("79 : userName = " + userName);
							System.out.println("79 : userId = " + userId);
							System.out.println("79 : memberId = " + memberId);
							System.out.println("79 : nhHierarchy = " + nhHierarchy);
							System.out.println("79 : nhId = " + nhId);
							System.out.println("79 : tableId = " + tableId);
							System.out.println("79 : view = " + view);
							System.out.println("79 : importTid = " + importTid);
							System.out.println("79 : exportTid = " + exportTid);
							System.out.println("79 : criticalLevel = " + criticalLevel);
							System.out.println("79 : lngCritical = " + lngCritical);
							System.out.println("79 : strComment = " + strComment);
							System.out.println("79 : rowCount = " + rowCount);
							System.out.println("79 : colCount = " + colCount);
							System.out.println("79 : cuboidRows = " + Arrays.toString(strCuboidRows));
							System.out.println("79 : cuboidCols = " + Arrays.toString(strCuboidCols));
							System.out.println("79 : cuboidColNames = " + Arrays.toString(strCuboidColNames));
							System.out.println("79 : jsonCuboidCellDataArr = " + jsonCuboidCellDataArr);


							String submitBuffer = "" ;
//							submitBuffer = AdminTasks.submitTable(connection, userId, userName, pwd, nhHierarchy, memberId, nhId, tableId, baselineId, view, importTid, exportTid, mode, synch, intCuboidRows,intCuboidCols, cuboidCellDataArr, strComment, lngCritical, criticalLevel, colCount, rowCount);
							submitBuffer = AdminTasks.submitTable(connection, userId, userName, pwd, nhHierarchy, memberId, nhId, tableId, baselineId, view, importTid, exportTid, mode, synch, strCuboidRows, strCuboidCols, strCuboidColNames, jsonCuboidCellDataArr, strComment, lngCritical, criticalLevel, colCount, rowCount);

							if (submitBuffer.toUpperCase().startsWith("FAILURE"))
							{
								String msg = submitBuffer.replace("FAILURE" + ContentDelimeter, "");
								map = new HashMap<String, String>();
								map.put("Status", "Failed");
								map.put("FailureReason", msg);
								map.put("DataElements", "0");
							...
							else
							{
								...
								List<String> suBuffer = new ArrayList<String>();
								suBuffer.add(submitBuffer);
										
								jsonReturn.accumulate("Elements", suBuffer);
								System.out.println(jsonReturn.toString());
							}
						...
						case 77:				// Link Export Table ..added by Rahul Varadkar on 24-FEBRUARY-2018
							System.out.println("case is 77 .........Link Export Table");

							String columnNames;
							String columnValues;
							String columnFormulas;

							userName = jObj.getString("userName");
							System.out.println("77 : userName = " + userName);

							pwd = jObj.getString("pwd");
							//userId = Integer.parseInt(jObj.getString("userId"));				// delete this
							//String userPassword = requestInfo[2];
							//memberId = Integer.parseInt(jObj.getString("memberId"));             //delete this
							nhHierarchy = jObj.getString("nhHierarchy");
							System.out.println("77 : nhHierarchy = " + nhHierarchy);

							//nhId = Integer.parseInt(jObj.getString("nhId"));
							tableId = Integer.parseInt(jObj.getString("cuboidId"));
							System.out.println("77 : tableId = " + tableId);
							
							colCount = Integer.parseInt(jObj.getString("colCount"));
							System.out.println("77 : colCount = " + colCount);
							
							rowCount = Integer.parseInt(jObj.getString("rowCount"));
							System.out.println("77 : rowCount = " + rowCount);
							
							columnNames = jObj.getString("columnNames");
							System.out.println("77 : columnNames = " + columnNames);

							columnValues = jObj.getString("columnValues");
							System.out.println("77 : columnValues = " + columnValues);

							columnFormulas = jObj.getString("columnFormulas");
							System.out.println("77 : columnValues = " + columnFormulas);


							String[] colNameArr = columnNames.split(Seperator);
							for (int r = 0; r < colNameArr.length; r++ )
							{
								System.out.println("colNameArr " + r +  ": " + colNameArr[r]);
							}

							String[] colValueArr = columnValues.split(ContentDelimeter);
							for (int r = 0; r < colValueArr.length; r++ )
							{
								System.out.println("colValueArr " + r +  ": " + colValueArr[r]);
							}

							String linkExportResponseBuffer = "" ;
							linkExportResponseBuffer = AdminTasks.LinkExportTable(connection, userName, pwd, nhHierarchy, tableId, colCount, rowCount, columnNames, columnValues, columnFormulas);

							System.out.println("Link Export buffer in xlEPortalService.java : " + linkExportResponseBuffer );

							if (linkExportResponseBuffer.toUpperCase().startsWith("FAILURE"))
							{
								String msg = linkExportResponseBuffer.replace("FAILURE" + ContentDelimeter, "");
								map = new HashMap<String, String>();
								map.put("Status", "Failed");
								map.put("FailureReason", msg);
								map.put("DataElements", "0");
							...
							else
							{
								...
								List<String> linkExpBuffer = new ArrayList<String>();
								linkExpBuffer.add(linkExportResponseBuffer);
										
								jsonReturn.accumulate("Elements", linkExpBuffer);
								System.out.println(jsonReturn.toString());
							}

							break;

						case 78:				// Create Cuboid..added by Rahul Varadkar on 25-FEBRUARY-2018
							System.out.println("case is 78 .........Create Cuboid...");

							userName = jObj.getString("userName");
							pwd = jObj.getString("pwd");
							nhHierarchy = jObj.getString("nhHierarchy");
							String cuboidName = jObj.getString("newCuboidName");
							desc = jObj.getString("newCuboidName");
							collabId = jObj.getInt("collabId");
							wbId = jObj.getInt("wbId");
							
							System.out.println("78 : userName = " + userName);
							System.out.println("78 : nhHierarchy = " + nhHierarchy);
							System.out.println("78 : newCuboidName = " + cuboidName);
							System.out.println("78 : collabId = " + collabId);
							System.out.println("78 : wbId = " + wbId);

							System.out.println("before calling  AdminTasks.CreateTableByUser.,.... ");
							System.out.println("bwcon.getUserId().... " + bwcon.getUserId());
							String createTableResponseBuffer = "" ;
							createTableResponseBuffer = AdminTasks.CreateTableByUser(connection, userName, pwd, nhHierarchy, collabId, wbId, cuboidName, desc);
							System.out.println("after calling  AdminTasks.CreateTableByUser :" + createTableResponseBuffer);

							if (createTableResponseBuffer.toUpperCase().startsWith("FAILURE"))
							{
								String msg = createTableResponseBuffer.replace("FAILURE" + ContentDelimeter, "");
								map = new HashMap<String, String>();
								map.put("Status", "Failed");
								map.put("FailureReason", msg);
								map.put("DataElements", "0");
							...
							else
							{
								...
								List<String> ctBuffer = new ArrayList<String>();
								ctBuffer.add(createTableResponseBuffer);
										
								jsonReturn.accumulate("Elements", ctBuffer);
								System.out.println(jsonReturn.toString());
							}
				...
				finally
				{
					...
					//String msg = new Gson().toJson(jsonReturn);
			...
			else
			{
				...
				String connErrMsg = "There is a Database connection problem. Login user 'admin' is missing. \nContact Boardwalk Administrator for support. ";

				map = new HashMap<String, String>();
				map.put("Status", "Failed");
				map.put("FailureReason", connErrMsg);
				map.put("DataElements", "0");
			...
			msgData.setMessageHeader("Failure");
...
/* MOVED TO boardwalk.eportal.AdminTasks.  REMOVE IT
