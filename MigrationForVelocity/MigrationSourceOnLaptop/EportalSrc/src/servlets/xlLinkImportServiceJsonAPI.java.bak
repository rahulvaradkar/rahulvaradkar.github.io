...
public class xlLinkImportServiceJsonAPI extends xlServiceJsonAPI implements SingleThreadModel
{
		...
		HttpServletResponse response)throws ServletException, IOException
	{
		...
		try
		{
			...
			// get the tableBuffer
			responseBuffer = TableViewManagerJsonAPI.getTableBuffer(connection, tableId, userId, memberId, nhId, baselineId, view, mode);
		...
		finally
		{
			...
			{
				System.out.println("Time to prepare response failure =0 ");
				commitResponseBuffer(responseBuffer, response);
				System.out.println("Time to prepare response = ");
