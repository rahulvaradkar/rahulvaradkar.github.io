...
import org.json.simple.JSONObject;					//Added from BLOCK-CHAIN DEMO WEB-APP
import org.json.simple.JSONArray;					//Added from BLOCK-CHAIN DEMO WEB-APP
import org.json.simple.parser.JSONParser;			//Added from BLOCK-CHAIN DEMO WEB-APP
import org.json.simple.parser.ParseException;		//Added from BLOCK-CHAIN DEMO WEB-APP
	...
	protected void oldSheetCheck(Connection connection, int tableId, int memberId, int userId, int exportTid, String view)	throws BoardwalkException
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				{
					stmt = connection.prepareStatement(" exec [BW_GET_TX_FOR_USER_VIEW] ?, ?, ?, ?, ? ");
					System.out.println("g2...... view query " + view);
					stmt.setInt(1, tableId);
					stmt.setInt(2, memberId);
					stmt.setInt(3, userId);
					stmt.setString(4, view);
					stmt.setInt(5, exportTid);
...
//			wout.close(); 
	...
	////Added from BLOCK-CHAIN DEMO WEB-APP
	protected JSONObject getRequestBufferJSON(ServletRequest request) throws IOException 
	{
		StringBuffer sb = null;
		try
		{
...
/*
			...
			sb = new StringBuffer();
			sb.append(out.toString("UTF-8"));
//			System.out.println(" getRequestBufferJSON " + out.toString("UTF-8"));
			zipIn.closeEntry();
			zipIn.close();
...
*/
		...
		//System.out.println(" getRequestBufferJSON sb.toString() " + sb.toString());
		JSONParser parser = new JSONParser();
		JSONObject jsonob = null;
		try {
			jsonob = (JSONObject) parser.parse(sb.toString());
		} catch (Exception e) {}
		//return sb;
		return jsonob;
	}

//Added by Rahul on 15-Sep-2016
	protected void commitResponseBufferJson(String buffer, ServletResponse response) throws IOException 
	{
		...
		String json = new Gson().toJson(b64String);
	...
	protected void commitResponseBufferJSON_(JSONObject jsonob, ServletResponse response) throws IOException 
	{
		response.setContentType("application/json");
		response.setCharacterEncoding("UTF-8");
		System.out.println(" getRequestBufferJSON jsonob " +jsonob);
