...
public class xlExportChangesServiceJsonSimulator extends xlServiceJsonSimulator implements SingleThreadModel
{
		...
		HttpServletResponse response)throws ServletException, IOException
	{
		...
		System.out.println("xlExportChangesServiceJsonSimulator: Time to read the buffer = " + getElapsedTime());
		...
		for (int ti = 0; ti < MAX_RETRY_ATTEMPTS; ti++)
		{
			try
			{
				...
				JSONParser parser = new JSONParser();
					...
					JSONArray RowData = (JSONArray) jsonob.get("RowData");
					JSONArray CellData = (JSONArray) jsonob.get("CellData");
					System.out.println("headerData==" + headerData);
					System.out.println("ColumnData==" + ColumnData);
					System.out.println("CellData==" + CellData);
					System.out.println("RowData==" + RowData);
					String[] stringArray = new String[headerData.size()];
				     for (int i = 0; i < headerData.size(); i++) {
				         stringArray[i]= (String) headerData.get(i);
				         
				          System.out.println("headerarr"+ stringArray[i]);
				     }
				     String[] stringArray1 = new String[ColumnData.size()];
				     for (int i = 0; i < ColumnData.size(); i++) {
				         //stringArray1[i]= (String) ColumnData.get(i);
				    	 stringArray1[i]=String.valueOf(ColumnData.get(i));
				          System.out.println("ColumnData"+ stringArray1[i]);
				     }
				     String[] stringArray2 = new String[RowData.size()-1];
				     for (int i = 0; i < RowData.size()-1; i++) {
				    	 stringArray2[i]=String.valueOf(RowData.get(i));
				         //stringArray2[i]= (String) RowData.get(i);
				         
				          System.out.println("RowData"+ stringArray2[i]);
				     }
				     String[] stringArray3 = new String[CellData.size()];
				     for (int i = 0; i < CellData.size(); i++) {
				         stringArray3[i]= (String) CellData.get(i);
				         
				          System.out.println("CellData"+ stringArray3[i]);
				     }
				   					
				//------
			processHeader(connection, stringArray);
			...
			//processHeader(connection, headerInfo);
				...
				processColumns(connection, tid,  stringArray1);
				//reader.getNextContent();
				processRows(connection, tid,stringArray2 );
				processCells(connection, tid,stringArray3 );
				...
				if (criteriaTableId > 0)
				{
					...
					String lsRowQuery  = TableViewManagerJsonSimulator.getRowQuery(connection, tableId, userId, criteriaTableId, true, "LATEST");
				...
				System.out.println("xlExportChangesServiceJsonSimulator: xlErrorCells.size() " + xlErrorCells.size());
				...
				responseToUpdateJson.add(rowIdHash.size());
				responseToUpdateJson.add(colIdHash.size());

				org.json.JSONObject responseToUpdateJsonObject = new org.json.JSONObject();
				responseToUpdateJsonObject.put("Status", responseToUpdateJson);
				
				System.out.println("  responseToUpdateJsonObject: " + responseToUpdateJsonObject);

				ti = MAX_RETRY_ATTEMPTS;
				response.getWriter().print(responseToUpdateJsonObject);
			}
			catch (SQLException sqe)
			{
				...
				if (sqe.getErrorCode() == 1205)
				{
					if (ti == MAX_RETRY_ATTEMPTS - 1)
					{
						...
						System.out.println("xlExportChangesServiceJsonSimulator:  Deadlock maximum attempts exhausted. Sending server busy message to client ");
					}
					else
					{
						System.out.println("xlExportChangesServiceJsonSimulator: Deadlock Attempt number = " + (ti + 1) + " out of max = " + MAX_RETRY_ATTEMPTS);
						try
						{
		...
		if (failureReason.length() == 0)
		{
			...
			System.out.println("xlExportChangesServiceJsonSimulator: Success : Time to prepare response = " + getElapsedTime());
		}
		else
		{
			failureReason = "FAILURE" + ContentDelimeter + failureReason;
			//commitResponseBuffer(failureReason, response);
			System.out.println("xlExportChangesServiceJsonSimulator: Failure : Time to prepare response = " + getElapsedTime());
			System.out.println("xlExportChangesServiceJsonSimulator: failureReason = " + failureReason);
		}
	}
	...
	System.out.println(" userId= " + headerInfo[0]);
		System.out.println("userName = " + headerInfo[1]);
		//System.out.println("userPassword = " + headerInfo[2]);
		System.out.println("memberId = " + headerInfo[3]);
		System.out.println("nhId = " + headerInfo[4]);
		System.out.println("tableId = " + headerInfo[5]);
		System.out.println("view = " + headerInfo[6]);
		System.out.println("numColumns = " + headerInfo[7]);
		System.out.println("numRows = " + headerInfo[8]);
		System.out.println("importTid = " + headerInfo[9]);
		System.out.println("exportTid = " + headerInfo[10]);
		System.out.println("isCritical = " + headerInfo[11]);
		System.out.println("criticalLevel = " + headerInfo[12]);
		System.out.println("asTxComment = " + headerInfo[13]);
		...
		*/
		...
		criteriaTableId = TableViewManagerJsonSimulator.getCriteriaTable(connection, tableId, userId);
		...
		if (!(view == null || view.trim().equals("") || view.equalsIgnoreCase("Latest")))
		{
			...
			String lsRowQuery = TableViewManagerJsonSimulator.getRowQuery(connection, tableId, userId, criteriaTableId, true, view);
		...
		if (accessTableId > 0)
		{
			...
			colCellAccess = TableViewManagerJsonSimulator.getColumnAcccess(connection, tableId, accessTableId, userId);
			...
			while (columnConditionalAccessIter.hasNext())
			{
				...
				if (colCellAccess.get(colId) instanceof String){
					...
					if(matcher.matches())
					{
						...
						if (rowSet == null)
						{
							String rowQuery = TableViewManagerJsonSimulator.getRowQuery(connection, 
									TableViewManagerJsonSimulator.getCriteriaForDynamicView(accessInstr), tableId, true);
