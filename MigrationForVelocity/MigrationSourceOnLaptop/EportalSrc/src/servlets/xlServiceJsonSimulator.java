 ...
 */
public abstract class xlServiceJsonSimulator extends HttpServlet implements SingleThreadModel
{
...
//			wout.close(); 
	...
	protected void commitResponseBuffer(String buffer, ServletResponse response) throws IOException 
	{
		...
		System.out.println(" -Uncompressed Response Buffer : Size = " + buffer);
		// write to ZipOutputStream
		ByteArrayOutputStream bos = new ByteArrayOutputStream ();
		ZipOutputStream out = new ZipOutputStream(bos);
		out.putNextEntry(new ZipEntry("response.txt"));
		...
		//made changes servletOut to out byme
		outPrint.print(b64String);
		outPrint.println(buffer);
		outPrint.close();
	}
}