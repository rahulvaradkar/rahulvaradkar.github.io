		 ...
		 boolean retValue = false;
		   try {
				...
				System.out.println("value of email address^^^^^^^^^"+emailAdd);
				 String query = "SELECT ACTIVE FROM BW_USER WHERE EMAIL_ADDRESS = '"+emailAdd+"' ";
		 ...
		 boolean retValue = false;
		   try {
				...
				System.out.println("value of userId^^^^^^^^^"+userId);
				 String query = "SELECT COUNT(BW_COLLAB.MEMBER_ID) COUNT FROM BW_COLLAB,BW_MEMBER WHERE BW_COLLAB.MEMBER_ID = BW_MEMBER.ID AND BW_MEMBER.USER_ID ='"+userId+"' ";
				 Statement stmt = connection.createStatement();
				 ResultSet rs   = stmt.executeQuery(query);
				 while (rs.next())
						{
